...
820g3 ::

   {
      ...
      mpv \
        --pause \
        --ytdl-format="140+160" \
        --ytdl-raw-options="$EXT" \
        --fs \
        --demuxer-max-bytes=64M \
        --no-resume-playback \
        "https://www.youtube.com/watch?v=b18tiLlgPDs"
   }

yt-dlp `<pick 1 out of 3 <https://www.reddit.com/r/mpv/comments/q759xc/comment/hhbva8j/>`__ ::

   mpv --demuxer-max-bytes=32m --script-opts=ytdl_hook-ytdl_path=yt-dlp 'ytdl://8Jvtl9KG9aY'
...
::
   ...
   streamlink \
      ...
      -p "mpv" \
      -a "--vo=gpu --hwdec=vaapi --pause --cache=yes --demuxer-max-bytes=128M" \
...
| |b| `ibus x KDE <https://wiki.archlinux.org/title/IBus#Integration>`__
| |b| `kkc <https://wiki.archlinux.org/title/Localization/Japanese#libkkc>`__
